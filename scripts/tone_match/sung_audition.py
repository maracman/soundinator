...
def _body_transfer_params(params: dict) -> dict:
    """Isolate the harmonic source/body consumer for the T-058 pair.

    The ordinary FIT render deliberately retains deterministic aspiration and
    onset noise because those are identity/watch dimensions.  Those broadband
    components are not generated by the per-harmonic Gaussian body law that
    T-058 measures, however, and a sparse source table can move weak harmonics
    below them.  Comparing that mixed signal made the paired assertion depend
    on source-row audibility even when the exact harmonic body consumer was
    unchanged.  The dedicated pair therefore keeps the fitted source surface
    and vowel body, but silences unrelated noise/onset/consonant components.
    Their own consuming assertions remain A-VOICE-03/A-VOICE-04/T-067.
    """

    result = params_for_mode(params, "fit")
    result.update({
        "toneBreath": 0.0,
        "voiceBreathSync": 0.0,
...
def build(
    ...
    for index, row in enumerate(selected):
        ...
        body_transfer_params = _body_transfer_params(params)
        bypass_params = {**body_transfer_params, "bodyBands": []}
        ...
        if body_transfer_out is not None:
            jobs.append({
                "params": body_transfer_params,
                "midi": row["midi"],
                "velocity": row["velocity"],
                "durationSec": row["durationSec"],
                "sampleRate": row.get("sampleRate", 44100),
                "out": str(body_transfer_out),
            })
            jobs.append({
        ...
        manifest.append({
            ...
            "bodyTransferRender": (
                str(body_transfer_out) if body_transfer_out is not None else None
            ),
    ...
    for row, trial in zip(selected, manifest):
        ...
        if trial["bodyBypassRender"] is not None:
            ...
            try:
                body_transfer_render = extract_features(
                    trial["bodyTransferRender"], active_duration_s=row["durationSec"],
                    expected_f0_hz=row["expectedF0Hz"],
                )
                ...
                common_mask = (
                    np.asarray(body_transfer_render.note.partial_snr_ok, dtype=bool)
                    & np.asarray(bypass.note.partial_snr_ok, dtype=bool)
                )
                transfer.update(compare_rendered_vowel_body_transfer(
                    body_transfer_render.note.partial_amps,
