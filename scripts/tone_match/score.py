...
def inharmonicity_comparison(ref: NoteAnalysis,
                             render: NoteAnalysis) -> dict[str, Any]:
    """T-037: compare near-zero string stretch in cents, not as a B ratio."""
    ref_ok = np.flatnonzero(np.asarray(ref.partial_snr_ok, dtype=bool)[:40]) + 1
    render_ok = np.flatnonzero(
        np.asarray(render.partial_snr_ok, dtype=bool)[:40]) + 1
    common_ok = np.intersect1d(ref_ok, render_ok, assume_unique=True)
    if ref.B is None or render.B is None or not common_ok.size:
        return {"applicable": False, "mode": None,
                "kind": "insufficient-evidence", "distance": 0.0}
    mode = int(common_ok.max())
    if mode < 2:
    ...
    def stretch_cents(B: float) -> float:
        bounded = max(0.0, B)
        return float(600 * math.log2(
            (1 + bounded * mode * mode) / (1 + bounded)))
    ...
    if ref_cents < 3.0:
        ...
        return {
            ...
            "mode": mode,
            "kind": "cents",
            "refStretchCents": ref_cents,
            "renderStretchCents": render_cents,
            "errorCents": error,
            "passed": error <= 3.0,
            # Preserve the existing feature normalizer: three cents equals
            # one perceptual unit for the near-harmonic branch.
            "distance": error * PERCEPTUAL_UNITS[
                "inharmonicity_log_ratio"] / 3.0,
        }
    factor = math.exp(abs(math.log(
        max(float(render.B), 1e-8) / max(float(ref.B), 1e-8))))
    return {
        "applicable": True,
        "mode": mode,
        "kind": "factor",
...
def compare_features(ref: FeatureBundle, render: FeatureBundle, weights: dict[str, float] | None = None) -> dict[str, Any]:
    ...
    b_distance = float(inharmonicity_comparison(
        ref.note, render.note)["distance"])
    number = lambda value, fallback=0.0: float(value) if isinstance(value, (int, float)) and np.isfinite(value) else fallback
