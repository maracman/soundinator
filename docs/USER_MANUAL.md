...
- `Surprise > Rest` creates a surprise rest that can be baked into the motif if incorporation succeeds.
...
Surprise controls unexpected changes at the motif-pass level.

| Control | Meaning | Musical analogy |
|---|---|---|
| Probability | Chance that a motif pass contains one surprised note. | One unexpected event in a repeated phrase. |
| Pitch | Surprise may change the note by a scale-degree offset. | Unexpected melodic note. |
| Octave | Surprise may jump by an octave. | Register break. |
| Formant | Intended to switch to another active vowel/formant. See the flag below: current playback may overwrite baked formant surprises. | Unexpected vowel colour. |
| Rhythm | Surprise may halve or double a note duration. | Unexpected rhythmic deformation. |
| Dynamics | Surprise may make the note very quiet or very loud. | Accent or ghost note. |
| Rest | Surprise may replace the note with silence. | Omitted note. |
...
3. If a surprise happens, `Incorporation` decides whether it becomes a new motif variant.
4. If baked, the sequence grows by appending one new base-length cycle with the surprised motif substituted for its source motif.
5. `Max baked` stops further growth after the selected number of baked variants.
...
In the current code, audible vowel movement mainly comes from the live `Formant change` path rather than from formants stored inside generated motifs.
...
1. `Sound Source > Formant` chooses the vowel/filter model.
   ...
   - `Formant change` changes the audible vowel label between notes. `Surprise > Formant` is intended to change the vowel label too, but the current playback path may overwrite baked formant surprises; see the flag below.
...
1. `Formant` appears in several places.
   ...
   - `Surprise > Formant` creates an unexpected vowel change that can be baked into the motif.
   - `Colour Distribution > Formant` shifts filter positions inside individual notes.
   Current code flag: playback assigns the audible formant from a live current-formant state after the surprise step, so baked formant surprises and formants stored in motifs may not persist audibly. `Formant change` still works as a live between-note vowel switch.
   Suggested wording: rename the chips to `Vowel palette`, Surprise to `Vowel`, and Colour Distribution to `Filter drift`. Also consider fixing the playback overwrite if baked formant surprises are meant to matter.
...
3. Rest controls exist in Rhythm and Surprise.
   - Rhythm rests are transient density/performance rests.
   - Surprise Rest can be baked into a motif.
