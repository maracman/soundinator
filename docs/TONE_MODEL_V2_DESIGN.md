...
**CH-B3 DONE (9e6b913, v145).** Fits landed for flute, clarinet, violin,
cello, trumpet, trombone, piano (JSON moved to web/static/ — web/data
stays gitignored). Curation: measured partials/material/partialB/
attackNoise/vibrato-character taken; hand dyn curves, envelopes and
vibratoProb kept (the samples measure Iowa's recording style there, not
the instrument). measured_profiles.js is generated by
scripts/gen_measured_profiles_module.py; synth.js merges at module init.
Vocal unmeasured (no solo-voice source) — hand profile stands.
Owner follow-up queued as the capstone phase: full browser QA sweep
(screenshots at several window sizes, fix quirks/inconsistencies) and
adjustable/resizable panels so a resized window never hides sections.
