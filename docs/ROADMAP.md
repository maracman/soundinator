...
| Q9 producer remainders: D2 roll parity (✏ pencil-add, ⌫/M/Q/arrows/⌥-arrows with audition), B2 multi-select (⇧click + rubber band + bulk gain/mute/duplicate/delete), C track headers (hue swatch → space mini-controls writing `track.space`, dB readout, drag reorder, delete confirm), F onboarding (3-step tour, ? shortcut overlay, "Key (root pitch)") | 07-07 brief | v160 |
