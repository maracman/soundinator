...
| Q10b musical typing: `kbdMidiNote` DAW-standard two-row map (A-row white keys, W-row black, Z/X octave, C/V velocity; 8 table tests), ⌨ Keys toolbar toggle + ⌘K, capture-phase keys pre-empt producer letter shortcuts while on, feeds `handleMidiEvent` so Q10 mapping/arm/bake apply unchanged, no Web MIDI permission needed; enabling auto-arms the selected track. Recording fixes: a take on an occupied lane spills onto a fresh "take" track (same voice) instead of the discard alert; monitor path drops the needless `initialise()` | 07-16 | `ad328c6` |
...
| Owner UI round 2: region toolbar pinned bottom, two-row contained track heads (190px), +8 bars at the ruler end, draggable playhead handle, scale/Space controls out of the session bar (reverb keys now VOICE-level — patches own their space; global space owns type+amount when on), compact browser rows, space designer laid out parallel to the timeline (cross-section+params in the head column, beat-aligned cylinder with region-bright threads + translucent head core, track-head/dot selection, ask-once anchoring, unanchored free drags via `space.static`), sub-note: inspector default 320 + envelope beside excitor knobs, head graphics in pad/thumb/cross-section | 07-07 owner review | v180 |
